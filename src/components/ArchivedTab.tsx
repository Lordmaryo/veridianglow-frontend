...
import { Archive, Ellipsis, PenLine, Trash } from "lucide-react";

const ArchivedTab = ({ products }: { products: Product[] }) => {
  ...
  const { deleteProduct, toggleArchivedProduct } = useProductStore();
  ...
  return (
    <div>
      {archivedProducts.length === 0 ? (
        <h2 className="font-semibold text-center pt-6 text-zinc-600">
          Saved but archived products are shown here
        </h2>
      ) : (
        <>

...
const MenCategoryPage = () => {
  const { menCategory } = useParams();
  const menCategoryProduct = menCategory?.replace(/-/g, " ");
  const { getProductsByMenCategory } = useProductStore();

  const [limit, setLimit] = useState(20);
  const [page, setPage] = useState(1);

  const { data } = useQuery({
    queryKey: ["for-men", page, limit, menCategoryProduct],
    queryFn: () =>
      getProductsByMenCategory(page, limit, menCategoryProduct || ""),
  });
